...
class TextCNN(object):
	"""
	embedding layer + conv-max-pooling layer + softmax layer for text classification
	"""
	...
	def lookup_layer_op(self):
		with tf.variable_scope('words'):
			...
			self.sentence_embeddings = tf.expand_dims(word_embeddings, -1)

	def conv_layer_op(self):
		with tf.variable_scope('conv-max-pooling'):
			pooled_outputs = []
			for i, filter_size in enumerate(self.filter_sizes):
				filter_shape = [filter_size, self.embedding_dim, 1, self.num_filters]
				W = tf.get_variable('W-%s' % i,
									shape = filter_shape,
									initializer = tf.truncated_normal_initializer(stddev = 0.1))
				b = tf.get_variable('b-%s' % i,
